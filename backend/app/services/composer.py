...
def _fit_wrapped(
    ...
    while size >= min_size:
        ...
        size -= step
    if best is None:
        # Never drop words: below min_size keep shrinking until every word fits;
        # as a last resort allow extra lines at the minimum size.
        size = min_size
        while size > 7:
            font = _load_font(font_path, size)
            lines = _wrap_text(draw, text, font, max_width, max_lines)
            if sum(len(line.split()) for line in lines) >= want_words:
                return font, lines
            size -= step
        font = _load_font(font_path, min_size)
        best = (font, _wrap_text(draw, text, font, max_width, max_lines + 2))
    return best
...
def _keyword_hit(keyword: str, haystack: str) -> bool:
    """Word-boundary match so short keywords never hit inside other words
    (e.g. 'eis' must not match 'fleisch')."""
    return re.search(rf"(?<![a-zäöüß]){re.escape(keyword)}(?![a-zäöüß])", haystack) is not None
...
def _resolve_builtin_asset(spec: PromotionSpec) -> Path | None:
    ...
    for asset_name, keywords in PRODUCT_ASSETS.items():
        if any(_keyword_hit(keyword, haystack) for keyword in keywords):
            ...
            if path.exists():
                return path
    if any(_keyword_hit(word, haystack) for word in FRUIT_WORDS):
        path = PRODUCT_ASSET_DIR / "mixed_fruit.png"
        return path if path.exists() else None
    if any(_keyword_hit(word, haystack) for word in VEGETABLE_WORDS):
...
def _draw_price_star(
    ...
    if include_statt and spec.old_price and not _is_event(spec):
    ...
    # Stronger tilts swing the star's valleys into the text zone — pinch the
    # price box so the digits never ride the rim.
    pinch = max(0.88, 1.0 - 0.022 * max(0.0, abs(rot_deg) - 8.0))
    _draw_price_value(draw, cx, price_cy, int(inner * 1.9 * pinch),
...
def _draw_price_disc(
    ...
    top_cursor = cy - int(inner * 0.70)
    if include_statt and spec.old_price and not _is_event(spec):
        old_text = f"statt {spec.old_price}"
        of = _fit_font_width(draw, old_text, FONT_PATH_BOLD, int(inner * 1.4), int(radius * 0.15), int(radius * 0.09))
        ob = draw.textbbox((0, 0), old_text, font=of)
        ow, oh = ob[2] - ob[0], ob[3] - ob[1]
        ox = cx - ow // 2
        draw.text((ox - ob[0], top_cursor - ob[1]), old_text, fill=_mix(accent, primary, 0.30), font=of)
        # A light warm strike: pure red would sink into the deep-green plate.
        draw.line((ox, top_cursor + oh * 0.5, ox + ow, top_cursor + oh * 0.5),
                  fill=_lighten(RED, 0.30), width=max(3, radius // 42))
        price_cy = cy + int(radius * 0.10)
        price_h = int(radius * 0.60)
    else:
        price_cy = cy
...
def _circle_rect_overlap(cx: int, cy: int, r: float, bbox) -> bool:
    if not bbox:
        return False
    x0, y0, x1, y1 = bbox
    nx = min(max(cx, x0), x1)
...
def _offer_column(
    ...
    *,
    ...
    if discount:
        ...
        if statt_outside:
            candidates = [
                (cx - dd * 0.87, cy - dd * 0.49),  # left shoulder of the seal
                (cx + dd * 0.87, cy - dd * 0.49),  # right shoulder
                (cx + dd * 0.93, cy + dd * 0.37),  # right, mid-height
            ]
        else:
            # The statt line lives inside the seal's top — keep the badge on
            # the upper points so it never covers the struck price.
            candidates = [
                (cx + dd * 0.59, cy - dd * 0.81),
                (cx - dd * 0.59, cy - dd * 0.81),
                (cx + dd * 0.93, cy + dd * 0.37),
            ]
...
def _draw_product_or_name(canvas, draw, spec, product_zone, name_color, fill_scale: float = 0.96):
    """Draw the product photo (returning its real bbox) or the wrapped name.

    The bbox lets layouts place badges like the discount burst so they never
    sit on the product photo. Returns None when only the name was drawn.
    """
    ...
    if product:
        ...
        return (
            product_zone.cx - product.width // 2,
            product_zone.cy - product.height // 2,
            product_zone.cx + product.width // 2,
            product_zone.cy + product.height // 2,
        )
    else:
...
def _draw_validity_tag(canvas, spec, cx, cy, height, accent, primary, angle: float = 0.0):
    ...
    _draw_tag(canvas, spec.validity, cx, cy, height, accent, primary, angle=angle, max_w=max_w, max_bottom=max_bottom)
...
def _layout_luxe(canvas: Image.Image, spec: PromotionSpec, fmt: FormatType):
    ...
    if product:
        ...
        prod_bbox = (pz.cx - product.width // 2, pz.cy - product.height // 2,
                     pz.cx + product.width // 2, pz.cy + product.height // 2)
    ...
    sr = int(w * (0.215 if tall else 0.200) * _seal_factor(spec))
    # Reserve the headline column too, so the burst and the Auffällig
    # sparkles never land on the big left-aligned name block.
    head_bbox = (margin, head_y, margin + int(w * 0.55), h - int(h * 0.155))
    _offer_column(
...
def _price_h_factor(spec: PromotionSpec) -> float:
    """Two-zone big-price height multiplier (market / colorblock), capped:
    pure Preisgröße."""
    return min(_price_size_mul(spec), 1.4)
...
def _layout_editorial(canvas: Image.Image, spec: PromotionSpec, fmt: FormatType):
    ...
    if tall:
        ...
        price_cx, price_cy, price_r = int(w * 0.74), int(h * 0.52), int(w * 0.205)
        head_y = int(h * 0.60)
    else:
        disc_cx, disc_cy, disc_r = int(w * 0.80), int(h * 0.18), int(w * 0.50)
        prod = Zone(int(w * 0.05), int(h * 0.12), int(w * 0.58), int(h * 0.46))
        price_cx, price_cy, price_r = int(w * 0.82), int(h * 0.60), int(w * 0.172)
        head_y = int(h * 0.62)
...
def _layout_colorblock(canvas: Image.Image, spec: PromotionSpec, fmt: FormatType):
    ...
    if tall:
        ...
        band_h = int(h * 0.44)
        canvas.alpha_composite(_vertical_gradient((w, band_h), _lighten(accent, 0.10), _darken(accent, 0.14)).convert("RGBA"), (0, 0))
        ...
        band_w = int(w * 0.47)
        canvas.alpha_composite(_vertical_gradient((band_w, h), _lighten(accent, 0.10), _darken(accent, 0.14)).convert("RGBA"), (0, 0))
    ...
    if tall:
        # Soft scrim over the band's top so the brand lockup stays readable even
        # when a boxy packshot slides underneath it.
        scrim_h = int(h * 0.18)
        scrim = Image.new("RGBA", (w, scrim_h), (*_darken(accent, 0.40), 0))
        scrim_alpha = Image.linear_gradient("L").rotate(180).resize((w, scrim_h)).point(lambda p: p * 165 // 255)
    ...
    if tall:
        price_h = int(h * 0.175 * _price_h_factor(spec))
        val_h = int(h * 0.024)
        ...
        vf = _fit_font_width(draw, meta, FONT_PATH_SEMIBOLD, price_w, val_h, int(val_h * 0.55))
        ...
        price_h = min(int(h * 0.165 * _price_h_factor(spec)), max(int(h * 0.09), avail))
...
def _layout_lifestyle(canvas: Image.Image, spec: PromotionSpec, fmt: FormatType):
    ...
    if tall:
        ...
        price_cx, price_cy, price_r = int(w * 0.74), int(h * 0.56), int(w * 0.178)
        head_y = int(h * 0.60)
    else:
        pz = Zone(int(w * 0.04), int(h * 0.13), int(w * 0.60), int(h * 0.45))
        price_cx, price_cy, price_r = int(w * 0.80), int(h * 0.66), int(w * 0.160)
        head_y = int(h * 0.64)
    ...
    sr = int(w * (0.215 if tall else 0.200))
...
def _layout_magazine(canvas: Image.Image, spec: PromotionSpec, fmt: FormatType):
    ...
    value = _offer_value(spec)
    has_old = bool(spec.old_price and not _is_event(spec))
    price_h = int(h * (0.135 if tall else 0.160) * _price_size_mul(spec))
...
def _layout_retro(canvas: Image.Image, spec: PromotionSpec, fmt: FormatType):
    ...
    if tall:
        ...
        star_cx, star_cy, star_r = int(w * 0.74), int(h * 0.52), int(w * 0.210)
        head_y = int(h * 0.66)
    else:
        pz = Zone(int(w * 0.10), int(h * 0.13), int(w * 0.56), int(h * 0.46))
        star_cx, star_cy, star_r = int(w * 0.77), int(h * 0.60), int(w * 0.195)
        head_y = int(h * 0.68)
...
def _layout_market_block(canvas, spec, fmt, *, page, block_a, block_b, accent, ink, muted,
    ...
    if tall:
        ...
        # when a boxy packshot slides underneath it.
        scrim_h = int(h * 0.20)
        scrim = Image.new("RGBA", (w, scrim_h), (*_darken(block_b, 0.35), 0))
        scrim_alpha = Image.linear_gradient("L").rotate(180).resize((w, scrim_h)).point(lambda p: p * 165 // 255)
        scrim.putalpha(scrim_alpha)
        canvas.alpha_composite(scrim, (0, 0))
    ...
    def _statt(px, py):
        ...
        if discount:
            ...
            if bx > w - int(w * 0.03) - br:
                bx = w - int(w * 0.045) - br
                by = py - int(br * 1.75)
            _draw_discount_burst_colored(canvas, discount, bx, by, br,
                                         price_color, _contrast_text(price_color))
        return py + int(h * 0.04)
...
def _draw_awning(canvas: Image.Image, bar_h: int, val_h: int,
                 color: tuple[int, int, int], stripe: tuple[int, int, int],
                 shadow: int = 90) -> int:
    """Market-stall awning: a solid canopy bar plus a striped valance whose
    scalloped hem hangs over the page. Returns the scallop overhang height."""
    w, h = canvas.size
    canvas.alpha_composite(_vertical_gradient((w, bar_h), _lighten(color, 0.14), color).convert("RGBA"), (0, 0))
    n = 11 if w <= h else 15   # odd stripe count so both ends close in colour
    stripe_w = w / n
    over = int(stripe_w * 0.5)
    if shadow > 0:
        sh = Image.new("RGBA", canvas.size, (0, 0, 0, 0))
        ImageDraw.Draw(sh).rectangle((0, bar_h, w, bar_h + val_h + over), fill=(18, 38, 22, min(140, shadow)))
        canvas.alpha_composite(sh.filter(ImageFilter.GaussianBlur(max(4, over // 2))))
    layer = Image.new("RGBA", canvas.size, (0, 0, 0, 0))
    d = ImageDraw.Draw(layer)
    for i in range(n):
        c = color if i % 2 == 0 else stripe
        x0, x1 = round(i * stripe_w), round((i + 1) * stripe_w)
        d.rectangle((x0, bar_h, x1, bar_h + val_h), fill=(*c, 255))
        d.ellipse((x0, bar_h + val_h - over, x1, bar_h + val_h + over), fill=(*c, 255))
    canvas.alpha_composite(layer)
    return over


def _layout_frischemarkt(canvas, spec, fmt):
    """Frischemarkt als Marktstand: grüne Markise mit gestreiftem Volant oben,
    das Produkt frei auf hellem Papier und ein runder Preis-Teller — bewusst
    ein anderes Bild als der Bio/Natur-Farbblock."""
    w, h = canvas.size
    draw = ImageDraw.Draw(canvas)
    ratio = h / w
    tall = ratio > 1.12
    ...
    ink = (30, 42, 34)
    muted = (108, 124, 110)
    paper = _tone_bg(spec, (250, 247, 239))

    canvas.paste(_vertical_gradient((w, h), _lighten(paper, 0.02), _darken(paper, 0.035)), (0, 0))
    draw = ImageDraw.Draw(canvas)
    ...
    badge_h = int(h * (0.030 if story else 0.033 if tall else 0.045))
    _draw_angebot_badge(canvas, int(w * 0.85), int(bar_h * 0.52), badge_h, white, deep, _offer_label(spec))

    content_top = bar_h + val_h + over
    tag_h = int(w * (0.046 if tall else 0.050))
    # Reserved so the burst / sparkles never climb onto the awning valance.
    awning_bbox = (0, 0, w, content_top + int(h * 0.005))

    # --- Columns: product on the open paper, round price plate right ---
    disc_r = int(w * (0.20 if story else 0.195 if tall else 0.185) * _seal_factor(spec))
    disc_cx = w - margin - int(disc_r * 1.06)
    disc_cy = int(h * (0.40 if tall else 0.44))
    ...
    # Without a photo the plate itself becomes the hero — centred and
    # corridor-capped — instead of the name appearing twice (zone + headline).
    has_photo = _resolve_product_asset(spec) is not None
    if has_photo:
        prod_x = int(w * 0.035)
        prod_right = disc_cx - int(disc_r * (0.78 if tall else 0.88)) - int(w * 0.02)
        prod_y = content_top + int(h * 0.012)
        ...
        prod_bbox = None
        disc_cx = w // 2
        disc_cy = int(h * (0.37 if story else 0.38 if tall else 0.42))
        corridor_top = content_top + int(h * 0.015)
        ...
        disc_r = min(int(w * (0.24 if story else 0.23 if tall else 0.22) * _seal_factor(spec)),
                     max(int(w * 0.16), fit_r))
        head = Zone(margin, head.y, w - margin * 2, head.h)
        tags_y = content_top + int(h * 0.02)

    tags_bbox = _draw_context_tags(canvas, spec, margin, tags_y, tag_h,
                                   force_region=tp.force_region, brand_bg=green, brand_fg=white)

    # --- Offer axis on the market plate ---
    final_cy = _offer_column(
        canvas, spec, cx=disc_cx, cy=disc_cy, r=disc_r,
        rim=deep, core=white, statt_color=muted,
        tag_bg=deep, tag_fg=white, tag_h=tag_h,
    ...
    if has_photo and final_cy + disc_r + tag_h * 2.6 > head.y:
        head = Zone(head.x, head.y, max(int(w * 0.32), disc_cx - int(disc_r * 1.08) - head.x), head.h)

    # --- Kicker + rule + headline on the shared left axis ---
    kicker = (spec.category or spec.origin or "Frisch vom Markt").strip()
...
def _layout_prospekt(canvas, spec, fmt):
    ...
    star_r = int(w * (0.215 if tall else 0.20) * _seal_factor(spec))
...
# ---------------------------------------------------------------------------
# Wochenangebote: mehrere Angebote als Prospekt-Raster
# ---------------------------------------------------------------------------

def _draw_multi_card(canvas, spec, item, zone, accent, blue, ink, muted):
    """Eine Angebotskarte im Wochenangebote-Raster: Motiv, Name, Preis, Rabatt."""
    draw = ImageDraw.Draw(canvas)
    radius = max(8, int(zone.h * 0.05))
    _solid_panel(
        canvas, (zone.x, zone.y, zone.x + zone.w, zone.y + zone.h), radius,
        (255, 255, 255), (250, 251, 253),
        top_accent=accent, accent_h=max(4, int(zone.h * 0.014)),
    )
    pad = int(zone.w * 0.07)
    img_zone = Zone(zone.x + pad, zone.y + pad + int(zone.h * 0.015), zone.w - pad * 2, int(zone.h * 0.42))
    mini = spec.model_copy(update={
        "product": item.name,
    ...
    _draw_soft_shadow(
        canvas, img_zone.cx - int(img_zone.w * 0.24), int(img_zone.y + img_zone.h * 0.88),
        int(img_zone.w * 0.48), max(6, int(img_zone.h * 0.07)), blur=12, intensity=55,
    )
    prod_bbox = _draw_product_or_name(canvas, draw, mini, img_zone, blue)

    discount = _discount_percent(item.old_price or "", item.price)
    if discount:
        br = max(14, int(zone.w * 0.105))
        bx, by = _clear_burst_pos([
            (zone.x + zone.w - int(zone.w * 0.14), zone.y + int(zone.w * 0.14)),   # card corner
            (zone.x + zone.w - int(zone.w * 0.12), zone.y + int(zone.w * 0.26)),   # a bit lower
            (zone.x + zone.w - int(zone.w * 0.13), zone.y + zone.h - int(zone.w * 0.14)),  # bottom-right
        ], br, prod_bbox, canvas.width, canvas.height)
        _draw_discount_burst(canvas, discount, bx, by, br)

    ty = zone.y + int(zone.h * 0.50)
    nf, nlines = _fit_wrapped(
        draw, item.name.upper(), FONT_PATH_EXTRABOLD, zone.w - pad * 2,
        int(zone.h * 0.17), int(zone.h * 0.072), int(zone.h * 0.042),
        max_lines=2, line_spacing=1.0,
    )
    ty = _draw_wrapped(draw, nlines, zone.x + pad, zone.w - pad * 2, ty, nf, ink, align="left", line_spacing=1.0)

    py = ty + int(zone.h * 0.025)
    if item.old_price:
        of = _load_font(FONT_PATH_BOLD, int(zone.h * 0.05))
        ot = f"statt {item.old_price}"
        ob = draw.textbbox((0, 0), ot, font=of)
        draw.text((zone.x + pad - ob[0], py - ob[1]), ot, fill=muted, font=of)
        strike_y = py + (ob[3] - ob[1]) * 0.55
        draw.line(
            (zone.x + pad, strike_y, zone.x + pad + (ob[2] - ob[0]), strike_y),
            fill=RED, width=max(2, zone.h // 200),
        )
        py += int((ob[3] - ob[1]) * 1.45)
    price_h = min(int(zone.h * 0.17), (zone.y + zone.h - pad) - py)
    if item.price and price_h > int(zone.h * 0.055):
        _draw_price_value(draw, zone.x + pad, py + price_h // 2, zone.w - pad * 2, price_h, item.price, RED, align="left")


def _layout_multi(canvas: Image.Image, spec: PromotionSpec, fmt: FormatType):
    """Wochenangebote-Plakat: blaues Kopfband, Titel mit Zeitraum und ein
    Prospekt-Raster aus 2-6 Angebotskarten. Der Footer kommt wie immer aus
    compose_promotion."""
    w, h = canvas.size
    draw = ImageDraw.Draw(canvas)
    ratio = h / w
    ...
    canvas.paste(_vertical_gradient((w, h), (255, 255, 255), (236, 242, 249)), (0, 0))
    draw = ImageDraw.Draw(canvas)

    band_h = int(h * (0.105 if ratio > 1.3 else 0.135))
    draw.rectangle((0, 0, w, band_h), fill=blue)
    draw.rectangle((0, band_h, w, band_h + max(3, int(h * 0.005))), fill=yellow)
    _draw_brand_lockup(
        canvas, margin, int(band_h * 0.5 - h * 0.04),
        int(h * (0.058 if ratio > 1.3 else 0.072)), yellow, sub_color=white, halo=True,
    )
    _draw_angebot_badge(
        canvas, int(w * 0.86), int(band_h * 0.52),
        int(h * (0.034 if ratio > 1.3 else 0.044)), yellow, blue, "ANGEBOTE",
    )

    ty = band_h + int(h * 0.02)
    tf, tlines = _fit_wrapped(
        draw, spec.product.upper(), FONT_PATH_EXTRABOLD, w - margin * 2,
        int(h * 0.085), int(h * 0.046), int(h * 0.026), max_lines=2, line_spacing=1.0,
    )
    ty = _draw_wrapped(draw, tlines, margin, w - margin * 2, ty, tf, ink, align="left", line_spacing=1.0)
    rule_y = ty + int(h * 0.008)
    draw.rectangle((margin, rule_y, margin + int(w * 0.12), rule_y + max(3, int(h * 0.008))), fill=yellow)
    vf = _load_font(FONT_PATH_BOLD, int(h * 0.019))
    vt = spec.validity.upper()
    vb = draw.textbbox((0, 0), vt, font=vf)
    vy = ty + int(h * 0.022)
    draw.text((margin - vb[0], vy - vb[1]), vt, fill=muted, font=vf)
    grid_top = vy + (vb[3] - vb[1]) + int(h * 0.022)

    content_bottom = h - int(h * 0.125) - int(h * 0.015)
    items = (spec.items or [])[:6]
    if ratio > 1.6:
    ...
    gap = int(w * 0.025)
    grid_w = w - margin * 2
    grid_h = max(1, content_bottom - grid_top)
    cell_w = (grid_w - gap * (cols - 1)) // cols
    cell_h = (grid_h - gap * (rows - 1)) // rows

    for idx, item in enumerate(items):
        r, c = divmod(idx, cols)
        in_row = cols if r < rows - 1 else (len(items) - r * cols)
        row_w = in_row * cell_w + (in_row - 1) * gap
        x0 = margin + (grid_w - row_w) // 2 + c * (cell_w + gap)
        y0 = grid_top + r * (cell_h + gap)
        _draw_multi_card(canvas, spec, item, Zone(x0, y0, cell_w, cell_h), yellow, blue, ink, muted)
...
def compose_promotion(
    ...
    if spec.campaign_kind.value == "multi":
        _layout_multi(canvas, spec, format_type)
    elif style == "ai":
